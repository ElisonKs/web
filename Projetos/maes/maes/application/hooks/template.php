...
class Template {
	...
	private function carregarDadosSessao($CI, &$html) {
		...
		$html	= str_replace("{RAZSCEMP}", $CI->session->userdata('razscemp'), $html);
		
		$html	= str_replace("{DISPLAY_ADMIN}", ($CI->session->userdata('admin_cambista') == 1)?'':'display: none;', $html);
		$html	= str_replace("{DISPLAY_CAMBISTA}", ($CI->session->userdata('bus_id_emp') == 1)?'display: none;':'', $html);
