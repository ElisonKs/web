...
class Aposta extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Aposta_Model', 'ApostaModel');
	...
	public function index($idcampeonato_jogo = NULL, $opcao_status = NULL) {
		...
		$dados['URL_REGISTRAR'] = site_url('aposta/novo');
		$dados['URL_GRAVAR_APOSTA'] = site_url('aposta/jogoxaposta');
		$dados['BLC_DADOS']     = array();
		$dados['idcampeonato_jogo'] = $idcampeonato_jogo;
		$dados['opcao_status']   = $opcao_status;
		$this->carregarDados($dados);
		$this->carregarCampeonatos($dados);
		$this->parser->parse('aposta_consulta', $dados);
	}
	
	public function novo($jogador_aposta,$valor_aposta) {
		  $aposta = array(
        				'id_aposta'            => '',
    					'idcambista_aposta'   =>  $this->session->userdata('id_cambista'),
        				'jogador_aposta'   =>  $jogador_aposta,
        				'valor_aposta' => $valor_aposta
    				  );
    		
    		 $this->ApostaModel->adicionar_aposta($aposta);
	}
	
	public function jogoxaposta($idjogo_jogapos,$valor_jogapos,$opcao_jogapos) {
		  $jogapos = array(
        				'id_jogapos'            => '',
    					'idaposta_jogapos'   =>  $this->ApostaModel->pegarUltima(),
        				'idjogo_jogapos'   =>  $idjogo_jogapos,
        				'valor_jogapos' => $valor_jogapos,
        				'opcao_jogapos' => $opcao_jogapos
    				  );
    		
    		 $this->ApostaModel->adicionar_jogapos($jogapos);
	}
	...
	private function carregarDados(&$dados) {
		$resultado = $this->JogoModel->getJogoCategoria($dados['idcampeonato_jogo'], 1);
