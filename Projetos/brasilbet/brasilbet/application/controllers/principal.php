...
//			$dados['DIV_PEDIDOS'] = '';
//		}
		if ($this->session->userdata('admin_cambista') == 1) {
			$this->carregarApostasporCambista($dados);
		}
//		$this->carregarGraficoVendasSemanais($dados);
                $dados['login_cambista'] = $this->session->userdata('login_cambista');
...
//	}
	...
	private function  carregarApostasporCambista(&$dados) {
		$dados['GRAFICO_APOSTA_POR_CAMBISTA'] = array();
		
		$resultado = $this->db->query(' SELECT id_cambista, '.
       								  ' 		  MAX(login_cambista) as login_cambista, '.
       								  ' 		  SUM(valor_aposta) as valor_aposta '.
									  ' FROM Aposta '.
									  ' LEFT OUTER JOIN cambista on id_cambista = idcambista_aposta '.
									  ' GROUP BY id_cambista '.
									  ' ORDER BY valor_aposta DESC ')->result();
		
		foreach ($resultado as $registro) {
			$dados['GRAFICO_APOSTA_POR_CAMBISTA'][] = array(
				"CAMBISTA"    => $this->retirar_caracteres($registro->login_cambista),
				"VALOR" => $registro->valor_aposta
			);
		}
	}
