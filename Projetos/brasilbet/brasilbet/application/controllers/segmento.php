...
//		$this->load->model('Empresa_Segmento_Model', 'EmpresaSegmentoModel');
	...
	public function salvar() {
		...
		if ($this->testarDados()) {
			...
			if (is_numeric($bus_id_seg)) {
				$this->session->set_flashdata('sucesso', 'Segmento salvo seg sucesso.');
				redirect('segmento');
			} else {
	...
	private function testarDados() {
		...
		if (empty($bus_icone_seg)) {
			$erros    = TRUE;
			$mensagem .= "- Ícone não preenchido.\n";
			$this->session->set_flashdata('ERRO_BUS_ICONE_SEG', 'has-error');
		}
		...
		if ($erros) {
			...
			$this->session->set_flashdata('bus_icone_seg', $bus_icone_seg);
		}
...
//		}
	...
	private function carregarDadosFlash(&$dados) {
		...
		$ERRO_BUS_ICONE_SEG     = $this->session->flashdata('ERRO_BUS_ICONE_SEG');
		$bus_descricao_seg      = $this->session->flashdata('bus_descricao_seg');
		$bus_icone_seg          = $this->session->flashdata('bus_icone_seg');

		if ($ERRO_BUS_SEG) {
			$dados['bus_descricao_seg']      = $bus_descricao_seg;
			$dados['bus_icone_seg']          = $bus_icone_seg;

			$dados['ERRO_BUS_DESCRICAO_SEG'] = $ERRO_BUS_DESCRICAO_SEG;
			$dados['ERRO_BUS_ICONE_SEG']     = $ERRO_BUS_ICONE_SEG;
		}
	}
}